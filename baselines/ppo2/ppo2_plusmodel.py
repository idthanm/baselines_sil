...
def learn(*, network, env, total_timesteps, model_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    model_runner = Runner(env=model_env, model=model, nsteps=nsteps, gamma=gamma, lam=lam)

    epinfobuf = deque(maxlen=100)
    model_epinfobuf = deque(maxlen=100)
    ...
    for update in range(1, nupdates+1):
        ...
        # —————————————— model part ————————————————
        tstart = time.perf_counter()
        if update % log_interval == 0 and is_mpi_root: logger.info('Stepping model...')
        # Get image batch
        model_obs, model_returns, model_masks, model_actions, model_values, \
        model_neglogpacs, model_states, model_epinfos = model_runner.run()

        if update % log_interval == 0 and is_mpi_root: logger.info('Done.')
        model_epinfobuf.extend(model_epinfos)
        # Here what we're going to do is for each minibatch calculate the loss and append it.
        model_mblossvals = []
        # Index of each element of batch_size
        # Create the indices array
        ...
        for _ in range(noptepochs):
            ...
            for start in range(0, nbatch, nbatch_train):
                ...
                mbinds = inds[start:end]
                model_slices = (arr[mbinds] for arr in (model_obs, model_returns, model_masks, model_actions,
                                                        model_values, model_neglogpacs))
                model_mblossvals.append(model.train(lrnow, cliprangenow, *model_slices))
        # Feedforward --> get losses --> update
        model_lossvals = np.mean(model_mblossvals, axis=0)
        # End timer
        tnow = time.perf_counter()
        # Calculate the fps (frame per second)
        model_fps = int(nbatch / (tnow - tstart))
        # —————————————— model part end————————————————
