...
def impala_cnn_plus_vector_fn(grids, vector, depths=[16, 32, 32], **conv_kwargs):
    grids = tf.transpose(grids, [0, 2, 3, 1])
    unscaled_images = tf.cast(grids, tf.float32)
    layer_num = 0
...
@register('cnn_plus_vector')
def cnn_plus_vector(**conv_kwargs):
    def network_fn(grid, vector):
        return cnn_plus_fc_fn(grid, vector, **conv_kwargs)
    return network_fn

@register('impala_cnn_plus_vector')
def impala_cnn_plus_vector(**conv_kwargs):
    def network_fn(grid, vector):
        return impala_cnn_plus_vector_fn(grid, vector, **conv_kwargs)
    return network_fn
